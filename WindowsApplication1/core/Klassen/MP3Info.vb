...
Public Class MP3Info
    ...
    Private Mp3HeaderPosition, Mp3ClearSize As Long
    ...
    '-------------------------------------------------------------------------------------------------------
    Public ReadOnly Property Mp3FileInfo As FileInfo
        Get
            Return Mp3FInfo
        End Get
    End Property
    ...
    Private Function GetMp3FileStream(ByVal MP3FilePath As String) As FileStream
        ...
        Else
            RaiseEvent Exception("Die MP3-Datei kann nicht gelesen werden '" & MP3FilePath & "'.")
            Exit Function
        End If
    ...
    Private Function GetMP3HeaderBytes(ByVal MP3FilePath As String) As Boolean
        ...
        Try
            While (MP3FileStream.Position + 4) <= MP3FileStream.Length
                ...
                If MP3FileStream.ReadByte = 255 Then
                    Mp3HeaderPosition = CInt(MP3FileStream.Position)
                    IsSyncByte = True
                End If
                ...
                While IsSyncByte
                    ...
                    With Mp3HeaderBitsStr
                        ...
                        Else
                            ...
                            Mp3ClearSize = CInt((MP3FileStream.Length - Mp3HeaderPosition - 128))
                            Return True
                        End If
                    End With
        ...
        Catch Exc As Exception
            RaiseEvent Exception(Exc.Message)
        Finally
            MP3FileStream.Close()
        End Try
    End Function
    ...
    '-------------------------------------------------------------------------------------------------------
    Private Function GetID3v1TAG(ByVal MP3FilePath As String) As ID3v1TAG
        Dim MP3FileStream As FileStream = GetMp3FileStream(MP3FilePath)
        Dim ByteToStringConv As New System.Text.UTF8Encoding()
    ...
    Public Function GetBitrate() As Integer
        ...
        Else
            ...
            Dim AverageFrameLenght As Integer = 0
            Dim AverageBitrate As Integer = 0

            'For more info see : http://www.multiweb.cz/twoinches/MP3inside.htm#MP3FileStructure

            With CXingHeader
                If .FileLenght > 0 AndAlso .NumberOfFrames > 0 Then
                    LastByte = CInt(Math.Round((.TOC(99) / 256) * .FileLenght, 0))
                    AverageFrameLenght = CInt(Math.Round(.FileLenght / .NumberOfFrames, 0))
    ...
    Public Function GetDuration() As Integer
        Dim _bitrate As Integer = GetBitrate()
        If Mp3ClearSize * 8 > 0 Then
            Dim _size As Long = CLng(Mp3ClearSize * 8)
            Dim Duration As Integer = 0

            If _bitrate > 0 AndAlso _size > 0 Then
                Duration = CInt(Math.Round(_size / _bitrate, 0))
                Return Duration
            Else
                Return 0
            End If
    ...
    Private Sub MP3Info_Exception(ex As String) Handles Me.Exception
